...
for i in range(5):
    ...
    with open(f'tensorflow_comparison_run_{i}_start.csv', 'w') as file:
        ...
        acc_val = model.evaluate(test)
        print(f'Accuracy: {acc}, Accuracy_val: {acc_val}')
        
        writer.writerow([44, acc, acc_val])

        # Train 45 epochs
        history = model.fit(train, epochs=45, validation_data=test)
        
        # Save history
        history_df = pd.DataFrame(history.history)
        history_df.to_csv(f'tensorflow_comparison_run_{i}.csv')
        ...
        for i in range(45):
            ...
            if i % 5 == 0:
                utils.my_csv.weights_to_csv(model, f'save_{i+45}')
